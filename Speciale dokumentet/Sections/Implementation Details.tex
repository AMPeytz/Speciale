...
\begin{assumption}\label{assumption: NTR-convex}
  ...
  The convex polytope is a polytope which bounds a convex set, and can therefore be defined by a convex hull. Therefore any linear combination of points in the \ac{NTR} or on the boundary of the \ac{NTR} is also in the \ac{NTR}.
  \textbf{Wikipedia reference her?}
\end{assumption}
...
and by assumption \ref{assumption: NTR-convex} we can approximate the \ac{NTR} by using the convex hull of these points, i.e connecting the vertices to form the outer hull.
...
However, i can sample bounds are the vertices of the simplex that cover the feasible space, and the midpoint between these.
This sample scheme leads to the following points in the $2$-dimensional case:
\[
  \begin{bmatrix}
    0 & 0 \\
    1 & 0 \\
